...
protected:
    ...
    std::vector<int16_t> GenerateSineWave(size_t num_samples, double frequency, int sample_rate,
                                          int16_t amplitude = 16000) {
        std::vector<int16_t> samples(num_samples);
        for (size_t i = 0; i < num_samples; ++i) {
            samples[i] = static_cast<int16_t>(amplitude *
                                              std::sin(2.0 * M_PI * frequency * i / sample_rate));
        }
        return samples;
    }
...
TEST_F(RNNoiseProcessorTest, VAD_SpeechReturnsHighProbability) {
    ...
    for (size_t i = 0; i < samples.size(); ++i) {
        ...
        samples[i] = static_cast<int16_t>(5000 * std::sin(2.0 * M_PI * 200 * t) +  // Fundamental
